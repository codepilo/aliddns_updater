...
def load_config():
    ...
    # 1. Provide an automated setup script or instruction to auto-generate the `~/.config/aliddns/config.json` template if it doesn't exist during run/initialization
    if not os.path.exists(args.config):
        ...
        try:
            ...
            is_zh = 'zh' in os.environ.get('LANG', '').lower()
            print("\n" + "="*65)
            if is_zh:
                print("🚀 [AliDDNS] 初始化向导")
                print("-" * 65)
                print("🎉 已为您生成默认配置文件，路径如下：")
                print(f"👉  {args.config}")
                print("\n⚠️  需要您的操作：")
                ...
                print("-" * 65)
                print("🎉 A default configuration file has been automatically generated at:")
                print(f"👉  {args.config}")
                print("\n⚠️  Action Required:")
    ...
    if not CONFIG['access_key_id'] or not CONFIG['access_secret'] or not CONFIG['domain']:
        ...
        print("\n" + "="*65)
        if is_zh:
            print("❌ [错误] 缺少必要的阿里云凭证信息！")
            print("请打开您的配置文件并填入完整信息：")
            print(f"👉  {args.config}")
            print("\n💡 提示：您也可以通过命令行参数传入凭证 (运行 --help 查看详情)")
        else:
            print("❌ [Error] Missing required Aliyun credentials!")
            print("Please open your configuration file and fill in your details:")
            print(f"👉  {args.config}")
            print("\n💡 Tip: You can also provide them via CLI arguments (run --help for details)")
        print("="*65 + "\n")
        sys.exit(1)
